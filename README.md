...
Validations have been performed by finding whether known key interactions, such as "dueling", are highlighted successfully. An example of dueling is shown above (with yellow dash lines), in which two engaged ants face each other to move back and forth several times. In ant communities, this aggressive interaction is already known to be an *informative* behavioral signal to predict the abnormal state of colonies *H. saltator* ants. Hence, BT is expected to be able to discover the occurrences of dueling as key local observations whilst IO-GEN is detecting abnormal states. 

<img src=imgs/fig5.jpg width="80%">

Four output examples are displayed above: (a) BT does not use thresholding for prioritisation of Grad-CAM outputs and (b)-(d) top 5\% heatmaps are observed near the ants engaged in dueling, even though spatial coordinate information was provided as input.
